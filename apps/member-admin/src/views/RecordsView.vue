...
import { cardIssueRecords, createMembers, gameConfigs, playRecords, transactionRecords } from "../data";
...
const filteredPlays = computed(() => playRecords.filter((item) => [item.id, item.braceletId, item.memberName, item.roomName].some((v) => v.toLowerCase().includes(query.value))));
...
const filteredMembers = computed(() => members.filter((item) => [item.name, item.account, item.phone, item.identityId].some((v) => v.toLowerCase().includes(query.value))));
...
<template>
  <section class="records-tabs glass-panel" aria-label="记录与数据分类">
    <button v-for="tab in tabs" :key="tab.id" type="button" :class="{ active: activeTab === tab.id }" @click="activeTab = tab.id; search = ''"><AppIcon :name="tab.icon" :size="18" /> {{ tab.label }}</button>
  </section>
  <section class="toolbar glass-panel">
    ...
    <span class="result-count"><AppIcon name="filter" :size="15" /> 当前为演示记录</span>
  </section>
  ...
  <section class="table-card glass-panel">
    <div class="data-table-wrap">
      ...
      <table v-else-if="activeTab === 'plays'" class="data-table"><thead><tr><th>记录编号</th><th>会员 / 手环</th><th>游玩房间</th><th>实时 / 最终积分</th><th>开始时间</th><th>结束时间</th></tr></thead><tbody><tr v-for="item in filteredPlays" :key="item.id"><td><code>{{ item.id }}</code></td><td>{{ item.memberName }}<small class="cell-sub">{{ item.braceletId }}</small></td><td><strong>{{ item.roomName }}</strong></td><td><strong class="score-value">{{ item.score.toLocaleString() }}</strong> 分</td><td>{{ item.startedAt }}</td><td><StatusBadge v-if="item.endedAt === '进行中'" tone="purple">进行中</StatusBadge><template v-else>{{ item.endedAt }}</template></td></tr></tbody></table>
