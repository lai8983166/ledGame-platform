...
<template>
  ...
  <section class="process-card glass-panel">
    ...
    <div class="process-form">
      <label class="form-field"><span>读卡器 UID <b>*</b></span><input v-model="chargeUid" data-testid="admin-charge-uid" inputmode="numeric" autocomplete="off" maxlength="32" autofocus placeholder="请刷手环，例：2283055618" @input="chargeError = ''" @keydown.enter.prevent="chargeWristband" /><small>刷卡后读卡器会自动发送回车，无需手动输入 UID。</small></label>
      <label class="form-field"><span>购买时长 <b>*</b></span><div class="duration-input"><input v-model.number="chargeMinutes" data-testid="admin-charge-minutes" type="number" min="1" max="1440" @input="chargeError = ''" /><span>分钟</span></div></label>
    ...
    <p v-if="chargeError" class="form-error" data-testid="admin-charge-error"><AppIcon name="alert" :size="16" />{{ chargeError }}</p>
    <footer><span data-testid="admin-platform-connection">{{ connected ? "已连接本机后端" : "等待本机后端连接" }}</span><button class="primary-button" data-testid="admin-charge-submit" type="button" :disabled="loading" @click="chargeWristband"><AppIcon name="card" :size="17" />确认充时</button></footer>
  </section>
